...
def patch_lmaxmix_for_dftu(incar_path):
    """
    Enforce ELECTRAFI-style LMAXMIX:
...
# -------------------------
# NEW: ID extraction + ML CHGCAR lookup for your new flat filename layout
# -------------------------

def extract_base_name_for_sad(chgcar_path: str) -> str:
    """
    Keep the script's original behavior for SAD/default directory naming:
      ".../00139449c9.chgcar.lz4" -> "00139449c9.chgcar"
    """
    return os.path.splitext(os.path.basename(str(chgcar_path)))[0]


def extract_base_id_for_matching(chgcar_path: str) -> str:
    """
    Extract the ID used to match ML files.

    Examples:
      ".../00139449c9.chgcar.lz4" -> "00139449c9"
      ".../55e7ed6c39.chgcar.lz4" -> "55e7ed6c39"
      ".../3250c5e043/CHGCAR"     -> "3250c5e043"
      ".../3250c5e043.CHGCAR"     -> "3250c5e043"
    """
    p = str(chgcar_path)
    base = os.path.basename(p)

    # old folder layout: .../<id>/CHGCAR
    if base.upper() == "CHGCAR":
    ...
    # strip chgcar extension (case-insensitive)
    if base.lower().endswith(".chgcar"):
        base = base[:-7]

    return base


def _looks_like_chgcar_file(fn: str) -> bool:
    u = fn.upper()
    return u == "CHGCAR" or u.endswith(".CHGCAR") or fn.lower().endswith(".chgcar")


def find_ml_chgcar_path(ml_root: str, base_id: str) -> Optional[str]:
    """
    Find the ML-predicted CHGCAR for `base_id`.

    Tries in order:
      1) Old layouts:
         - <ml_root>/<base_id>/CHGCAR
         - <ml_root>/<base_id>.chgcar
         - <ml_root>/<base_id>.CHGCAR
         - <ml_root>/<base_id>
      2) New layout (your current one):
         - anywhere under <ml_root>, a file named: "<base_id>_*.CHGCAR" (or .chgcar)
    ...
    # ---- 1) old/direct candidates ----
    candidates = [
        os.path.join(ml_root, base_id, "CHGCAR"),
        os.path.join(ml_root, f"{base_id}.chgcar"),
        os.path.join(ml_root, f"{base_id}.CHGCAR"),
        os.path.join(ml_root, base_id),
    ]
    for c in candidates:
        if os.path.isfile(c):
            return c

    # ---- 2) new layout: recursive search for "<base_id>_*.CHGCAR" ----
    prefix = base_id + "_"
    matches: List[str] = []
    for root, _, files in os.walk(ml_root):
        for fn in files:
            # must start with "<base_id>_"
            if not fn.startswith(prefix):
                continue
            if not _looks_like_chgcar_file(fn):
                continue
    ...
    matches.sort()
    if len(matches) > 1:
        print(f"⚠️ Multiple ML CHGCAR matches for base_id={base_id}. Using first lexicographically:")
        for m in matches[:10]:
            print(f"    {m}")
        if len(matches) > 10:
...
def run_vasp_pymatgen(atoms,
    ...
    similar to the ELECTRAFI GNoME setup.
    ...
    # INCAR overrides relative to MP defaults (ELECTRAFI-style static settings)
    user_incar = {
    ...
    # Patch LMAXMIX for DFT+U, ELECTRAFI style
...
def build_ml_initialized_chgcar(
    ...
    Build a CHGCAR for ML-init on GNoME:
      ...
      - Parse *ml_chgcar_path* (ELECTRAFI CHGCAR) manually using the grid-dim line.
...
# -------------------------
# Main execution: ML-INIT on GNoME (ICHARG=1 from ELECTRAFI-patched CHGCAR)
# -------------------------
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description=(
            ...
            "replaced by a ELECTRAFI prediction (ICHARG=1). "
    ...
    parser.add_argument(
        ...
        dest="RESULTS_CSV",
        required=True,
        help="CSV file to append ML-init results to (e.g. ELECTRAFI_ML_INIT_GNOME.csv)",
    )
    parser.add_argument(
        "--sad_runs_dir",
        dest="SAD_RUNS_DIR",
    ...
    parser.add_argument(
        ...
        required=True,
        help=(
            "Root directory containing ELECTRAFI-predicted CHGCARs.\n"
            "Supported layouts:\n"
            "  - old: <root>/<id>/CHGCAR or <root>/<id>.chgcar\n"
            "  - new: any file anywhere under <root> named '<id>_*.CHGCAR' (or .chgcar)\n"
            "Where <id> is derived from the task CSV CHGCAR_PATH (e.g. '00139449c9' from '00139449c9.chgcar.lz4')."
        ),
    )
    parser.add_argument(
        "--vasp_cmd",
    ...
    # Keep original SAD/default naming behavior
    base_name = extract_base_name_for_sad(chgcar_path)   # e.g. "00139449c9.chgcar"
    base_id   = extract_base_id_for_matching(chgcar_path)  # e.g. "00139449c9"
    print(f"Derived base_name(for SAD dirs)={base_name} ; base_id(for ML match)={base_id}")
    ...
    # ---- NEW: find ML CHGCAR using base_id prefix match ----
    ml_chgcar_path = find_ml_chgcar_path(args.ML_CHGCAR_ROOT, base_id)
    if ml_chgcar_path is None:
        print(f"❌ ML-predicted CHGCAR not found for base_id={base_id} under ML_CHGCAR_ROOT={args.ML_CHGCAR_ROOT}")
        print("   Expected either:")
        print(f"     - {args.ML_CHGCAR_ROOT}/{base_id}/CHGCAR")
        print(f"     - {args.ML_CHGCAR_ROOT}/{base_id}.chgcar (or .CHGCAR)")
        print(f"     - OR any file named '{base_id}_*.CHGCAR' anywhere under ML_CHGCAR_ROOT")
        df.at[idx, "performed_ml_init"] = True
        df.to_csv(args.CSV_PATH, index=False)
        raise SystemExit(3)
    ...
    # W&B init
    wandb.init(
        project="VASP_SCF_ELECTRAFI_GNOME",
