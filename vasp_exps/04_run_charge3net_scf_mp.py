...
def _read_ml_grid_flat(ml_chgcar_path: str, n_needed: int):
    """
    Read n_needed floats from a ChargE3Net-style CHGCAR:
...
def build_ml_initialized_chgcar(
    ...
    Build a CHGCAR for ML-init:
      ...
      - Parse *ml_chgcar_path* (ChargE3Net-style CHGCAR) manually using the grid-dim line.
...
# --------------------------------------------------------------------------------------
# Main execution: ML-INIT (ICHARG=1 from ChargE3Net-patched CHGCAR)
# --------------------------------------------------------------------------------------
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description=(
            ...
            "replaced by a ChargE3Net prediction (ICHARG=1), and rescaled "
    ...
    pending_mask = (
        ...
        (df_slice["performed_default"] == True) &
        nonmag_mask
    )
    ...
    # Path to ChargE3Net-predicted CHGCAR
    ml_chgcar_root = args.ML_CHGCAR_ROOT
    ml_chgcar_path = os.path.join(ml_chgcar_root, mpid, "CHGCAR")

    if not os.path.isfile(ml_chgcar_path):
        print(f"❌ ML-predicted CHGCAR not found at {ml_chgcar_path}")
        df.at[idx, "performed_ml_init"] = True
        df.to_csv(args.CSV_PATH, index=False)
        raise SystemExit(3)
    ...
    # W&B init
    wandb.init(
        project="VASP_SCF_ChargE3Net_MP_NoAUG",
