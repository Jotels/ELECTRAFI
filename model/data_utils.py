...
import os
from tools.density_conversions import get_density
import numpy as np
    ...
    def __getitem__(self, idx):
        ...
        while not valid:
            try:
                ...
                density, struc, n_elec, grid_dict = get_density(path)
                valid = True
            except Exception as e:
                print(f"Error loading {cd_file}: {e}")
                idx = np.random.randint(0, len(self.file_list) - 1)
        if self.ood_name is None:
            sample = (density, struc, n_elec, grid_dict, cd_file, None)
        else:
            sample = (density, struc, n_elec, grid_dict, cd_file, self.ood_name)
...
class DensityStream(IterableDataset):
    ...
    def _iter_files(self, files):
        for cd_file in files:
            path = os.path.join(self.dens_path, cd_file)
            try:
                density, struc, n_elec, grid_dict = get_density(path)  # your lz4->tmp->read
                if self.ood_name is None:
                    yield (density, struc, n_elec, grid_dict, cd_file, None)
                else:
                    yield (density, struc, n_elec, grid_dict, cd_file, self.ood_name)
            except Exception as e:
