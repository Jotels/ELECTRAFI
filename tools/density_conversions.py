...
def normalize_density(n_elec: int,
    ...
    if volume is not None:
        dv = volume / (grid_dict['nx'] * grid_dict['ny'] * grid_dict['nz'])
    else:
...
def chgcar_to_cd(chgcar_file):
    ...
    spin_polarized = bool(getattr(vcd, "chgdiff", [])) and (len(vcd.chgdiff) > 0)
    ...
    def to_3d_total(x):
        ...
        if x.ndim == 1:
            ...
            if ngridpts is None:
                raise ValueError("Flat charge array but missing ngridpts; cannot infer shape.")
            ...
            raise ValueError(f"Cannot infer 3D shape from flat array of size {flat.size} (N={N}).")
    ...
    try:
        chg_arr = to_3d_total(chg[-1] if isinstance(chg, list) else chg)
    except Exception:
        ...
        nx, ny, nz = map(int, ngridpts)
        N = nx * ny * nz
        flat = np.asarray(chg).ravel()
        if flat.size == 2 * N:
            chg_arr = (flat[:N] + flat[N:]).reshape(nx, ny, nz, order="F")
        elif flat.size == N:
            chg_arr = flat.reshape(nx, ny, nz, order="F")
        else:
            raise ValueError(f"Unexpected flat buffer size {flat.size}; expected N={N} or 2N={2 * N}.")

    cd = torch.as_tensor(chg_arr, dtype=torch.float32)
    return cd, atoms
...
def get_density(path):
    density, struc = chgcar_to_cd(path)
    grid_dict = {"nx": density.shape[0],
                      "ny": density.shape[1],
                      "nz": density.shape[2]}
    n_elec = check_number_of_electrons(struc, density, grid_dict=grid_dict)
    return density, struc, n_elec, grid_dict
